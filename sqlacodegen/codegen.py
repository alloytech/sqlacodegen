...
{models}"""
    ...
    def render_table_proxy(self, model):
        """
        This will generate a proxy variable to t_tablename and fix the type of tablename.c:
        ...
        rendered += f'class {proxy_type}(Table):  # pylint: disable=too-many-ancestors,abstract-method\n'
        rendered += f'{self.indentation}c: {columns_type}\n'
        rendered += '\n\n'

        # force mypy to believe table_name's c has our defined columns_type
        rendered += f'{model.table.name}: {proxy_type} = _t_{model.table.name} # type: ignore'
        rendered += '\n\n'
        return rendered
